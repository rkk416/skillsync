...
public final class AnalyticsView extends VBox {
    ...
    private AnalyticsData loadData() {
        ...
        try {
            ...
            Map<String, Number> placementStatus = orFallback(service.generatePlacementStatusDistribution(), FALLBACK_PLACEMENT);
            ...
            long teamCount = service.countActiveTeams();
            ...
            double successRate = service.countPlacementSuccessRate();

            Map<String, Metric> kpis = buildKpis(studentCount, companyCount, skillCount,
                    recCount, teamCount, collabWeek, eligibility, successRate, appCount);

            List<String> topSkills = skills.entrySet().stream()
                    .limit(5).map(e -> e.getKey() + "  –  " + e.getValue() + " students").toList();

            java.util.List<String> liveCompanies = service.getTopCompanyNames();
            List<String> topCompanies = liveCompanies.isEmpty()
                    ? List.of("Northstar Labs", "CloudWave Technologies", "Aster Systems", "DataCore Analytics", "Fusion AI")
                    : liveCompanies;

            List<String> activeTeams = List.of(
                    "Active teams on platform:  " + teamCount,
                    "Team applications tracked:  " + appCount,
                    "Collaboration events (week):  " + collabWeek);
            List<String> insights = buildInsights(eligibility, collabWeek, recCount, successRate);

            return new AnalyticsData(kpis, skills, industry, placementStatus, recommendations, collab, learning,
    ...
    private Map<String, Metric> buildKpis(long students, long companies, long skills,
                                           long recs, long teams, long collabWeek,
                                           double eligibility, double successRate, long applications) {
        ...
        kpis.put("Active Teams",      new Metric(String.valueOf(teams),      "Collaboration teams on platform",          ViewFactory.WARNING));
        kpis.put("Applications",      new Metric(String.valueOf(applications),"Total placement applications submitted",  ViewFactory.ACCENT));
        kpis.put("Eligibility Rate",  new Metric(eligibility + "%",         "Students meeting ≥1 company criteria",    ViewFactory.SUCCESS));
        kpis.put("Success Rate",      new Metric(successRate + "%",         "Applications resulting in selection",     ViewFactory.PRIMARY));
        return kpis;
    }

    private List<String> buildInsights(double eligibility, long collabWeek, long recs, double successRate) {
        return List.of(
                eligibility >= 50
                ...
                successRate > 0
                        ? "🏆  Placement success rate stands at " + successRate + "% — track applications for best outcomes."
